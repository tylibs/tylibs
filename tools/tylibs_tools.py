...
#
# By default, downloaded tools will be installed under $HOME/.tylibs directory
# (%USERPROFILE%/.tylibs on Windows). This path can be modified by setting
# TYLIBS_TOOLS_PATH variable prior to running this tool.
#
# Users do not need to interact with this script directly. In TYLIBS root directory,
# install.sh (.bat) and export.sh (.bat) scripts are provided to invoke this script.
#
# Usage:
#
# * To install the tools, run `tylibs_tools.py install`.
#
# * To install the Python environment, run `tylibs_tools.py install-python-env`.
#
# * To start using the tools, run `eval "$(tylibs_tools.py export)"` — this will update
...
TYLIBS_MAINTAINER = os.environ.get("TYLIBS_MAINTAINER") or False
TODO_MESSAGE = "TODO"
...
PYTHON_VENV_DIR_TEMPLATE = "tylibs{}_py{}_env"
PYTHON_VER_MAJOR_MINOR = f"{sys.version_info.major}.{sys.version_info.minor}"
VENV_VER_FILE = "tylibs_version.txt"
...
class GlobalVarsStore:
    ...
    _string_properties = ["tylibs_path", "tylibs_tools_path", "tools_json"]
...
class TYLIBSToolDownload(object):
    """
...
class TYLIBSToolVersion(object):
    """
    ...
    def __init__(self, version: str, status: str) -> None:
        ...
        self.status = status
        self.downloads: OrderedDict[str, TYLIBSToolDownload] = OrderedDict()
        self.latest = False

    def __lt__(self, other: "TYLIBSToolVersion") -> bool:
        if self.status != other.status:
            return self.status > other.status
        else:
            assert not (
                self.status == TYLIBSToolVersion.STATUS_RECOMMENDED
                and other.status == TYLIBSToolVersion.STATUS_RECOMMENDED
            )
            return self.version < other.version

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, TYLIBSToolVersion):
    ...
    def add_download(
        ...
        """
        Add download entry of type TYLIBSToolDownload into self.downloads.
        """
        self.downloads[platform_name] = TYLIBSToolDownload(
            platform_name, url, size, sha256, rename_dist
        )

    def get_download_for_platform(
        self, platform_name: Optional[str]
    ) -> Optional[TYLIBSToolDownload]:
        """
...
class TYLIBSTool(object):
    """
    Used to store info about TYLIBS tools from tools.json file in a Python-accesible form.
    The term "TYLIBS tool" is used for e.g. CMake, ninja, QUEMU and toolchains.
    """
    ...
    def __init__(
        ...
        self.options = TYLIBSToolOptions(
            version_cmd,
            version_regex,
            version_regex_replace,
            is_executable,
    ...
    def copy_for_platform(self, platform: str) -> "TYLIBSTool":
        """
        Copy the TYLIBSTool record in respect to given platform (e.g. apply platform overrides).
        """
    ...
    def _update_current_options(self) -> None:
        ...
        """
        self._current_options = TYLIBSToolOptions(*self.options)
    ...
    def drop_versions(self) -> None:
        ...
        """
        self.versions: Dict[str, TYLIBSToolVersion] = OrderedDict()

    def add_version(self, version: TYLIBSToolVersion) -> None:
        """
        Add new TYLIBSVersion to self.versions.
        """
        assert type(version) is TYLIBSToolVersion
    ...
    def get_path(self) -> str:
        ...
        """
        return os.path.join(g.tylibs_tools_path, "tools", self.name)
    ...
    def get_export_vars(self, version: str) -> Dict[str, str]:
        ...
        for k, v in self._current_options.export_vars.items():  # type: ignore
            ...
            # Remove condition in TYLIBS-10292
    ...
    def get_recommended_version(self) -> Optional[str]:
        ...
        recommended_versions = [
            ...
            if v.status == TYLIBSToolVersion.STATUS_RECOMMENDED
    ...
    def get_preferred_installed_version(self) -> Optional[str]:
        ...
        recommended_versions = [
            ...
            if self.versions[k].status == TYLIBSToolVersion.STATUS_RECOMMENDED
    ...
    def find_installed_versions(self) -> None:
        """
        Checks whether the tool can be found in PATH and in GlobalVarsStore.tylibs_tools_path.
        ...
        # Now check all the versions installed in GlobalVarsStore.tylibs_tools_path
    ...
    def download(self, version: str) -> None:
        ...
        local_path = os.path.join(g.tylibs_tools_path, "dist", archive_name)
    ...
    def install(self, version: str) -> None:
        ...
        Should always be called after TYLIBSTool.download().
        """
        ...
        archive_path = os.path.join(g.tylibs_tools_path, "dist", archive_name)
    ...
    @staticmethod
    def check_download_file(download_obj: TYLIBSToolDownload, local_path: str) -> bool:
        """
    ...
    def from_json(
        ...
        """
        Create TYLIBSTool class instance form its JSON dump.
        """
        ...
        for version_dict in versions:  # type: ignore
            ...
            if (
                ...
                and version_status not in TYLIBSToolVersion.STATUS_VALUES
            ):
                raise RuntimeError(
                    "tool {} version {} status is not one of {}",
                    tool_name,
                    version,
                    TYLIBSToolVersion.STATUS_VALUES,
                )

            version_obj = TYLIBSToolVersion(version, version_status)
            for platform_id, platform_dict in version_dict.items():  # type: ignore
                ...
                if version_status == TYLIBSToolVersion.STATUS_RECOMMENDED:
        ...
        for platform_id, version_list in recommended_versions.items():
            ...
            if install != TYLIBSTool.INSTALL_NEVER and len(recommended_versions) == 0:
                raise RuntimeError(
...
class TYLIBSEnvEncoder(JSONEncoder):
    """
    TYLIBSEnvEncoder is used for encoding TYLIBSEnv, TYLIBSRecord, SelectedTYLIBSRecord classes to JSON in readable format. Not as (__main__.TYLIBSRecord object at '0x7fcxx').
...
class TYLIBSRecord:
    ...
    def __eq__(self, other: object) -> bool:
        if not isinstance(other, TYLIBSRecord):
            return False
        return all(
    ...
    @classmethod
    def get_active_tylibs_record(cls) -> "TYLIBSRecord":
        tylibs_record_obj = cls()
        tylibs_record_obj.version = get_tylibs_version()
        tylibs_record_obj.path = g.tylibs_path
        return tylibs_record_obj

    @classmethod
    def get_tylibs_record_from_dict(cls, record_dict: Dict[str, Any]) -> "TYLIBSRecord":
        tylibs_record_obj = cls()
        try:
            tylibs_record_obj.version = record_dict["version"]
            tylibs_record_obj.path = record_dict["path"]
        except KeyError:
        ...
        tylibs_record_obj.update_features(record_dict.get("features", []))
        tylibs_record_obj.extend_targets(record_dict.get("targets", []))
...
class TYLIBSEnv:
    ...
    def __init__(self) -> None:
        active_tylibs_id = active_repo_id()
        self.tylibs_installed: Dict[str, TYLIBSRecord] = {
            active_tylibs_id: TYLIBSRecord.get_active_tylibs_record()
        }

    def __iter__(self):  # type: ignore
        yield from {
            "tylibsInstalled": self.tylibs_installed,
        }.items()

    def __str__(self) -> str:
        return json.dumps(dict(self), cls=TYLIBSEnvEncoder, ensure_ascii=False, indent=4)  # type: ignore
    ...
    def save(self) -> None:
        """
        Diff current class instance with instance loaded from TYLIBS_ENV_FILE and save only if are different.
        """
        # It is enough to compare just active records because others can't be touched by the running script
        if (
            self.get_active_tylibs_record()
            != self.get_tylibs_env().get_active_tylibs_record()
        ):
            tylibs_env_file_path = os.path.join(g.tylibs_tools_path, TYLIBS_ENV_FILE)
            try:
                if g.tylibs_tools_path:  # mypy fix for Optional[str] in the next call
                    # the directory doesn't exist if this is run on a clean system the first time
                    mkdir_p(g.tylibs_tools_path)
                with open(tylibs_env_file_path, "w", encoding="utf-8") as w:
                    info(f"Updating {tylibs_env_file_path}")
                    json.dump(dict(self), w, cls=TYLIBSEnvEncoder, ensure_ascii=False, indent=4)  # type: ignore
            except (IOError, OSError):
                if not os.access(g.tylibs_tools_path, os.W_OK):
                    raise OSError(
                        f"TYLIBS_TOOLS_PATH {g.tylibs_tools_path} is not accessible to write. "
                        "Required changes have not been saved"
                    )
                raise OSError(
                    f"File {tylibs_env_file_path} is not accessible to write or corrupted. Required changes have not been saved"
                )

    def get_active_tylibs_record(self) -> TYLIBSRecord:
        return self.tylibs_installed[active_repo_id()]

    @classmethod
    def get_tylibs_env(cls) -> "TYLIBSEnv":
        """
        TYLIBSEnv class is used to process TYLIBS_ENV_FILE file. The constructor is therefore called only in this method that loads the file and checks its contents.
        """
        tylibs_env_obj = cls()
        try:
            tylibs_env_file_path = os.path.join(g.tylibs_tools_path, TYLIBS_ENV_FILE)
            with open(tylibs_env_file_path, "r", encoding="utf-8") as tylibs_env_file:
                tylibs_env_json = json.load(tylibs_env_file)
                ...
                try:
                    ...
                    for tylibs in tylibs_installed:
                        try:
                            tylibs_installed_verified[tylibs] = (
                                TYLIBSRecord.get_tylibs_record_from_dict(
                                    tylibs_installed[tylibs]
                                )
                            )
            ...
            # If no, empty or not-accessible to read TYLIBS_ENV_FILE found, use default values from constructor
            pass

        return tylibs_env_obj
...
class ENVState:
    ...
    * tylibs_variables - loaded TYLIBS variables from file
    """

    env_key = "TYLIBS_DEACTIVATE_FILE_PATH"
    deactivate_file_path = os.environ.get(env_key, "")

    def __init__(self) -> None:
        self.tylibs_variables: Dict[str, Any] = {}
        ...
        if cls.deactivate_file_path:
            try:
                with open(cls.deactivate_file_path, "r", encoding="utf-8") as fp:
                    env_state_obj.tylibs_variables = json.load(fp)
            except (IOError, OSError, ValueError):
                pass
        ...
        try:
            if self.deactivate_file_path:
                with open(self.deactivate_file_path, "w", encoding="utf-8") as w:
                    json.dump(self.tylibs_variables, w, ensure_ascii=False, indent=4)  # type: ignore
            else:
                with tempfile.NamedTemporaryFile(
                    delete=False, suffix=f"tylibs_{str(os.getppid())}"
                ) as fp:
                    self.deactivate_file_path = fp.name
                    fp.write(
                        json.dumps(
                            self.tylibs_variables, ensure_ascii=False, indent=4
                        ).encode("utf-8")
                    )
        except (IOError, OSError):
            warn(
...
def load_tools_info() -> Dict[str, TYLIBSTool]:
    """
...
def parse_tools_info_json(tools_info):  # type: ignore
    ...
    Returns a dictionary of tools (key: tool name, value: TYLIBSTool object).
    """
    ...
    for tool_dict in tools_array:
        tool = TYLIBSTool.from_json(tool_dict)
        tools_dict[tool.name] = tool

    return tools_dict


def dump_tools_json(tools_info: Optional[Dict[str, TYLIBSTool]]):  # type: ignore
    """
...
def get_tylibs_version() -> str:
    ...
    version_file_path = os.path.join(g.tylibs_path, "version.txt")
    if os.path.exists(version_file_path):
        with open(version_file_path, "r", encoding="utf-8") as version_file:
            tylibs_version_str = version_file.read()

        match = re.match(r"^v([0-9]+\.[0-9]+).*", tylibs_version_str)
        if match:
            tylibs_version = match.group(1)

    if tylibs_version is None:
        try:
            with open(
                os.path.join(
                    g.tylibs_path,
                ...
                m = re.search(
                    r"^#define\s+TYLIBS_VERSION_MAJOR\s+(\d+).+?^#define\s+TYLIBS_VERSION_MINOR\s+(\d+)",
                    f.read(),
                    re.DOTALL | re.MULTILINE,
                )
                if m:
                    tylibs_version = ".".join((m.group(1), m.group(2)))
                else:
                    fatal("Reading TYLIBS version from C header file failed!")
                    raise SystemExit(1)
        except Exception as e:
            fatal(f"It is not possible to determine the TYLIBS version: {e}")
            raise SystemExit(1)
    return tylibs_version
...
def get_python_env_path() -> Tuple[str, str, str, str]:
    ...
    """
    tylibs_version = get_tylibs_version()
    tylibs_python_env_path = os.getenv("TYLIBS_PYTHON_ENV_PATH") or os.path.join(
        g.tylibs_tools_path,
        "python_env",
        PYTHON_VENV_DIR_TEMPLATE.format(tylibs_version, PYTHON_VER_MAJOR_MINOR),
    )

    python_exe, subdir = get_python_exe_and_subdir()
    tylibs_python_export_path = os.path.join(tylibs_python_env_path, subdir)
    virtualenv_python = os.path.join(tylibs_python_export_path, python_exe)

    return (
        tylibs_python_env_path,
        tylibs_python_export_path,
        virtualenv_python,
        tylibs_version,
    )
    ...
    if "required" in tools_spec:
        tools.extend(
            [
                ...
                if v.get_install_type() == TYLIBSTool.INSTALL_ALWAYS and k not in tools
            ]
        )
        ...
        tools.extend(
            [
                k
                for k, v in overall_tools.items()
                if v.get_install_type() != TYLIBSTool.INSTALL_NEVER and k not in tools
            ]
        )
...
def add_and_check_targets(tylibs_env_obj: TYLIBSEnv, targets_str: str) -> List[str]:
    """
    Define targets from targets_str, check that the target names are valid and add them to tylibs_env_obj.
    """
    targets = parse_targets_arg(targets_str)
    tylibs_env_obj.get_active_tylibs_record().extend_targets(targets)
    return tylibs_env_obj.get_active_tylibs_record().targets
...
def feature_to_requirements_path(feature: str) -> str:
    ...
    """
    return os.path.join(
        g.tylibs_path, "tools", "requirements", f"requirements.{feature}.txt"
    )


def process_and_check_features(
    tylibs_env_obj: TYLIBSEnv, features_str: str
) -> List[str]:
    """
    Check whether new feature is valid. If yes, update features in active TYLIBS record.
    """
    new_features = []
    remove_features = []
    invalid_features = []

    for new_feature_candidate in [
        feature for feature in features_str.split(",") if feature != ""
    ]:
    ...
    if invalid_features:
        fatal(
            f'The following selected features are not valid: {", ".join(invalid_features)}'
        )
        raise SystemExit(1)

    tylibs_env_obj.get_active_tylibs_record().update_features(
        tuple(new_features), tuple(remove_features)
    )
    return tylibs_env_obj.get_active_tylibs_record().features
...
def filter_tools_info(
    tylibs_env_obj: TYLIBSEnv, tools_info: Dict[str, TYLIBSTool]
) -> Dict[str, TYLIBSTool]:
    """
    Filter tools info; return only those targets which:
        * are installable (install type is INSTALL_ALWAYS or INSTALL_ON_REQUEST)
        * support at least one target from active TYLIBS record
    """
    targets = tylibs_env_obj.get_active_tylibs_record().targets
    if not targets:
        return tools_info
    else:
        filtered_tools_spec = {
            ...
            if (
                v.get_install_type() == TYLIBSTool.INSTALL_ALWAYS
                or v.get_install_type() == TYLIBSTool.INSTALL_ON_REQUEST
            )
            and (
...
def add_variables_to_deactivate_file(
    ...
    new_tylibs_vars["PATH"] = new_tylibs_vars.get("PATH", [])
    args_add_paths_extras = vars(args).get(
        "add_paths_extras"
    )  # remove mypy error with args
    new_tylibs_vars["PATH"] = (
        new_tylibs_vars["PATH"] + args_add_paths_extras.split(":")
        if args_add_paths_extras
        else new_tylibs_vars["PATH"]
    )

    env_state_obj = ENVState.get_env_state()

    if env_state_obj.tylibs_variables:
        exported_tylibs_vars = env_state_obj.tylibs_variables
        new_tylibs_vars["PATH"] = list(
            set(new_tylibs_vars["PATH"] + exported_tylibs_vars.get("PATH", []))
        )  # remove duplicates
        env_state_obj.tylibs_variables = dict(
            exported_tylibs_vars, **new_tylibs_vars
        )  # merge two dicts
    else:
        env_state_obj.tylibs_variables = new_tylibs_vars
...
def print_deactivate_statement(args: List[str]) -> None:
    """
    Deactivate statement is sequence of commands, that remove TYLIBS global variables from environment,
    so the environment gets to the state it was before calling export.{sh/fish} script.
    """
    env_state_obj = ENVState.get_env_state()
    if not env_state_obj.tylibs_variables:
        return
    unset_vars = env_state_obj.tylibs_variables
    ...
    env_state_obj.tylibs_variables.clear()
    env_state_obj.save()
    return
...
def active_repo_id() -> str:
    ...
    """
    try:
        # g.tylibs_path is forcefully casted to str just to make type linters happy
        return f"{str(g.tylibs_path)}-v{get_tylibs_version()}"
    except ReferenceError:
        return f"UNKNOWN_PATH-v{get_tylibs_version()}"
...
def list_default(args):  # type: ignore
    ...
    for name, tool in tools_info.items():
        if tool.get_install_type() == TYLIBSTool.INSTALL_NEVER:
            continue
        optional_str = (
            " (optional)"
            if tool.get_install_type() == TYLIBSTool.INSTALL_ON_REQUEST
            else ""
        )
...
def action_check(args):  # type: ignore
    ...
    """
    tools_info = load_tools_info()
    tools_info = filter_tools_info(TYLIBSEnv.get_tylibs_env(), tools_info)
    not_found_list = []
    tool_error = False
    ...
    for name, tool in tools_info.items():
        ...
        if (
            ...
            and tool.get_install_type() == TYLIBSTool.INSTALL_ALWAYS
        ):
...
# The following function is used in process_tool which is a part of the action_export.
def handle_supported_or_deprecated_version(tool: TYLIBSTool, tool_name: str) -> None:
    """
    Prints info if supported, but not recommended or deprecated version of the tool is used.
    """
    version_obj: TYLIBSToolVersion = tool.versions[tool.version_in_path]  # type: ignore
    if version_obj.status == TYLIBSToolVersion.STATUS_SUPPORTED:
        info(
    ...
    elif version_obj.status == TYLIBSToolVersion.STATUS_DEPRECATED:
        warn(
...
def handle_missing_versions(
    tool: TYLIBSTool, tool_name: str, install_cmd: str, prefer_system_hint: str
) -> None:
    """
...
def process_tool(
    ...
    if not tool.versions_installed:
        if tool.get_install_type() == TYLIBSTool.INSTALL_ALWAYS:
            if os.getenv("TYLIBS_SKIP_TOOLS_CHECK", "0") == "1":
                warn(
                    f"Tool {tool_name} is not installed and TYLIBS_SKIP_TOOLS_CHECK is set. "
...
def check_python_venv_compatibility(
    tylibs_python_env_path: str, tylibs_version: str
) -> None:
    try:
        with open(
            os.path.join(tylibs_python_env_path, VENV_VER_FILE), "r", encoding="utf-8"
        ) as f:
            read_tylibs_version = f.read().strip()
...
def action_export(args: Any) -> None:
    ...
    prefer_system_hint = (
        ""
        if TYLIBS_TOOLS_EXPORT_CMD
        else f" To use it, run '{self_restart_cmd} export --prefer-system'"
    )
    install_cmd = (
        to_shell_specific_paths([TYLIBS_TOOLS_INSTALL_CMD])[0]
        if TYLIBS_TOOLS_INSTALL_CMD
    ...
    tylibs_python_env_path, tylibs_python_export_path, virtualenv_python, _ = (
        get_python_env_path()
    )
    if os.path.exists(virtualenv_python):
        tylibs_python_env_path = to_shell_specific_paths([tylibs_python_env_path])[0]
        if os.getenv("TYLIBS_PYTHON_ENV_PATH") != tylibs_python_env_path:
            export_vars["TYLIBS_PYTHON_ENV_PATH"] = to_shell_specific_paths(
                [tylibs_python_env_path]
            )[0]
        if current_path:  # getenv can return None
            # Remove condition in TYLIBS-10292
            if sys.platform != "win32" or tylibs_python_export_path not in current_path:
                paths_to_export.append(tylibs_python_export_path)

    tylibs_version = get_tylibs_version()
    if os.getenv("ESP_TYLIBS_VERSION") != tylibs_version:
        export_vars["ESP_TYLIBS_VERSION"] = tylibs_version

    check_python_venv_compatibility(tylibs_python_env_path, tylibs_version)

    tylibs_tools_dir = os.path.join(g.tylibs_path, "tools")  # type: ignore
    tylibs_tools_dir = to_shell_specific_paths([tylibs_tools_dir])[0]
    if current_path:
        # Remove condition in TYLIBS-10292
        if sys.platform != "win32" or tylibs_tools_dir not in current_path:
            paths_to_export.append(tylibs_tools_dir)
    ...
    if paths_to_export:
        ...
        # tylibs-exe has to be before \tools in PATH
        if sys.platform == "win32":
            paths_to_check = rf'{export_vars["PATH"]}{os.environ["PATH"]}'
            try:
                if paths_to_check.index(r"\tools;") < paths_to_check.index(
                    r"\tylibs-exe"
                ):
                    warn(
                ...
                fatal(
                    f"Both of the directories (..\\tylibs-exe\\.. and ..\\tools) has to be in the PATH:\n\n{paths_to_check}\n"
                )
...
def get_tylibs_download_url_apply_mirrors(
...
def apply_mirror_prefix_map(args: Any, tylibs_download_url: str) -> str:
    """
    Rewrite URL for given tylibs_download_url.
    If --mirror-prefix-map flag or TYLIBS_MIRROR_PREFIX_MAP environment variable is given.
    """
    new_url = tylibs_download_url
    mirror_prefix_map = None
    mirror_prefix_map_env = os.getenv("TYLIBS_MIRROR_PREFIX_MAP")
    if mirror_prefix_map_env:
        mirror_prefix_map = mirror_prefix_map_env.split(";")
    if TYLIBS_MAINTAINER and args and args.mirror_prefix_map:
        if mirror_prefix_map:
            warn(
                "Both TYLIBS_MIRROR_PREFIX_MAP environment variable and --mirror-prefix-map flag are specified, "
    ...
    if mirror_prefix_map:
        for item in mirror_prefix_map:
            ...
            new_url = re.sub(search, replace, tylibs_download_url)
            if new_url != tylibs_download_url:
                info(f"Changed download URL: {tylibs_download_url} => {new_url}")
                break
    return new_url


def apply_github_assets_option(tylibs_download_url: str) -> str:
    """
    Rewrite URL for given tylibs_download_url if the download URL is an https://github.com/ URL and the variable
    TYLIBS_GITHUB_ASSETS is set. The github.com part of the URL will be replaced.
    """
    new_url = tylibs_download_url
    github_assets = os.environ.get("TYLIBS_GITHUB_ASSETS", "").strip()
    if not github_assets:
        # no TYLIBS_GITHUB_ASSETS or variable exists but is empty
        return new_url

    # check no URL qualifier in the mirror URL
    if "://" in github_assets:
        fatal(
            "TYLIBS_GITHUB_ASSETS shouldn't include any URL qualifier, https:// is assumed"
        )
        raise SystemExit(1)
    ...
    new_url = re.sub(
        r"^https://github.com/", f"https://{github_assets}/", tylibs_download_url
    )
    if new_url != tylibs_download_url:
        info(f"Using GitHub assets mirror for URL: {tylibs_download_url} => {new_url}")
    return new_url
...
def get_tools_spec_and_platform_info(
    ...
    tools_spec: List[str],
    quiet: bool = False,
) -> Tuple[List[str], Dict[str, TYLIBSTool]]:
    """
    Returns tools_spec list and dict of tools for selected platform in form tool_name : TYLIBSTool object.
...
def action_download(args):  # type: ignore
    """
    Saves current TYLIBS environment and for every tools in tools_spec, downloads the right archive for tools version and target platform, if possible.
    If not, prints appropriate message to stderr and raise SystemExit() exception.
    """
    tools_spec = parse_tools_arg(args.tools)

    targets: List[str] = []
    # Saving TYLIBSEnv::targets for selected ESP_targets if all tools have been specified
    if "required" in tools_spec or "all" in tools_spec:
        tylibs_env_obj = TYLIBSEnv.get_tylibs_env()
        targets = add_and_check_targets(tylibs_env_obj, args.targets)
        try:
            tylibs_env_obj.save()
    ...
    for tool_spec in tools_spec:
        ...
        _tylibs_tool_obj = tool_obj.versions[tool_version].get_download_for_platform(
            platform
        )
        _tylibs_tool_obj.url = get_tylibs_download_url_apply_mirrors(
            args, _tylibs_tool_obj.url
        )
...
def action_install(args):  # type: ignore
    """
    Saves current TYLIBS environment and for every tool in tools_spec, installs the tool from the archive downloaded before, if possible.
...
def get_wheels_dir() -> Optional[str]:
    """
    Gets path for tylibs-python-wheels package.
    """
    tools_info = load_tools_info()
    wheels_package_name = "tylibs-python-wheels"
...
def get_requirements(new_features: str) -> List[str]:
    ...
    """
    tylibs_env_obj = TYLIBSEnv.get_tylibs_env()
    features = process_and_check_features(tylibs_env_obj, new_features)
    try:
        tylibs_env_obj.save()
    except OSError as err:
...
def get_constraints(tylibs_version: str, online: bool = True) -> str:
    """
    Download constraints file for specified TYLIBS vversion if it was not downloaded recently (1 day),
    check success and place it in constraints file location.
    """
    tylibs_download_url = get_tylibs_download_url_apply_mirrors()
    constraint_file = f"esptylibs.constraints.v{tylibs_version}.txt"
    constraint_path = os.path.join(g.tylibs_tools_path, constraint_file)
    constraint_url = "/".join([tylibs_download_url, constraint_file])
...
def action_install_python_env(args):  # type: ignore
    ...
    tylibs_python_env_path, _, virtualenv_python, tylibs_version = get_python_env_path()
    ...
    if is_virtualenv and (not os.path.exists(tylibs_python_env_path) or reinstall):
        fatal(
    ...
    if reinstall and os.path.exists(tylibs_python_env_path):
        warn(f"Removing the existing Python environment in {tylibs_python_env_path}")
        shutil.rmtree(tylibs_python_env_path)
    ...
    if os.path.exists(virtualenv_python):
        ...
        if (
            ...
            info(f"Creating a new Python environment in {tylibs_python_env_path}")

            try:
                environ_tylibs_python_env_path = os.environ["TYLIBS_PYTHON_ENV_PATH"]
                correct_env_path = environ_tylibs_python_env_path.endswith(
                    PYTHON_VENV_DIR_TEMPLATE.format(
                        tylibs_version, PYTHON_VER_MAJOR_MINOR
                    )
                )
                if not correct_env_path and re.search(
                    PYTHON_VENV_DIR_TEMPLATE.format(r"\d+\.\d+", r"\d+\.\d+"),
                    environ_tylibs_python_env_path,
                ):
                    warn(
                ...
                # if TYLIBS_PYTHON_ENV_PATH not defined then the above checks can be skipped
                pass
            ...
            try:
                with open(
                    os.path.join(tylibs_python_env_path, VENV_VER_FILE),
                    "w",
                    encoding="utf-8",
                ) as f:
            ...
            install_legacy_python_virtualenv(tylibs_python_env_path)
    ...
    constraint_file = get_constraints(tylibs_version) if use_constraints else None
...
def action_check_python_dependencies(args):  # type: ignore
    ...
    _, _, virtualenv_python, tylibs_version = get_python_env_path()

    if not os.path.isfile(virtualenv_python):
        fatal(
            f'{virtualenv_python} doesn\'t exist! Please run the install script or "tylibs_tools.py install-python-env" in order to create it'
        )
        raise SystemExit(1)

    if use_constraints:
        constr_path = get_constraints(
            tylibs_version, online=False
    ...
    # The dependency checker will be invoked with virtualenv_python. tylibs_tools.py could have been invoked with a
    # different one, therefore, importing is not a suitable option.
    dep_check_cmd = [
        virtualenv_python,
        os.path.join(g.tylibs_path, "tools", "check_python_dependencies.py"),
    ]
...
class ChecksumFileParser:
    ...
    def __init__(self, filename_prefix: str, url: str) -> None:
        ...
        sha256_file_tmp = os.path.join(
            g.tylibs_tools_path, "tools", "add-version.sha256.tmp"
        )
...
def action_add_version(args: Any) -> None:
    """
    Adds new version of the tool to TYLIBSTool entry together with download entry and updating json dump.
    """
    tools_info = load_tools_info()
    tool_name = args.tool
    ...
    version_status = TYLIBSToolVersion.STATUS_SUPPORTED
    if args.override and len(tool_obj.versions):
        tool_obj.drop_versions()
        version_status = TYLIBSToolVersion.STATUS_RECOMMENDED
    ...
    if not args.output:
        args.output = os.path.join(g.tylibs_path, TOOLS_FILE_NEW)  # type: ignore
...
def action_rewrite(args):  # type: ignore
    ...
    json_str = dump_tools_json(tools_info)
    if not args.output:
        args.output = os.path.join(g.tylibs_path, TOOLS_FILE_NEW)
    with open(args.output, "w", encoding="utf-8") as f:
        f.write(json_str)
        f.write("\n")
    info(f"Wrote output to {args.output}")
...
def action_uninstall(args: Any) -> None:
    ...
    """
    tools_info = load_tools_info()
    tools_path = os.path.join(g.tylibs_tools_path, "tools")
    dist_path = os.path.join(g.tylibs_tools_path, "dist")
    ...
    KEEP_WIN_TOOLS = ["tylibs-git", "tylibs-python"]
        ...
        if unused_tools_versions:
            print(
                "For removing old versions of {} use command '{} {} {}'".format(
                    ...
                    os.path.join(g.tylibs_path, "tools", "tylibs_tools.py"),
                    "uninstall",
                )
            )
        return
...
def action_validate(args):  # type: ignore
    ...
    with open(
        os.path.join(g.tylibs_path, TOOLS_FILE), "r", encoding="utf-8"
    ) as tools_file:
        tools_json = json.load(tools_file)

    with open(
        os.path.join(g.tylibs_path, TOOLS_SCHEMA_FILE), "r", encoding="utf-8"
    ) as schema_file:
...
def action_gen_doc(args):  # type: ignore
    ...
    for tool_name, tool_obj in tools_info.items():
        ...
        if f"{tylibs_gh_url}/tree" in info_url:
            info_url = re.sub(
                f"{tylibs_gh_url}/tree/\\w+/(.*)", r":tylibs:`\1`", info_url
            )
...
.. include:: tylibs-tools-notes.inc
...
""".rstrip().format(
        ...
        for platform_name in sorted(tool_obj.get_supported_platforms()):
            ...
            if install_type == TYLIBSTool.INSTALL_NEVER:
                continue
            elif install_type == TYLIBSTool.INSTALL_ALWAYS:
                install_type_str = "required"
            elif install_type == TYLIBSTool.INSTALL_ON_REQUEST:
...
def action_check_tool_supported(args: Any) -> None:
    """
    Print "True"/"False" to stdout as a result that tool is supported in TYLIBS.
...
def main(argv: List[str]) -> None:
    ...
    export.add_argument(
        "--add_paths_extras",
        help="Add tylibs-related path extras for deactivate option",
    )
    ...
    no_constraints_default = os.environ.get(
        "TYLIBS_PYTHON_CHECK_CONSTRAINTS", ""
    ).lower() in ["0", "n", "no"]

    if TYLIBS_MAINTAINER:
    ...
    install_python_env.add_argument(
        ...
        help=(
            ...
            "package versions by yourself. It can be set with the TYLIBS_PYTHON_CHECK_CONSTRAINTS "
    ...
    check_python_dependencies.add_argument(
        ...
        "to manage package versions by yourself. It can be set with the TYLIBS_PYTHON_CHECK_CONSTRAINTS "
        "environment variable.",
    )

    if os.environ.get("TYLIBS_TOOLS_VERSION_HELPER"):
        check_tool_supported = subparsers.add_parser(
            "check-tool-supported",
            help='Check that selected tool is compatible with TYLIBS. Writes "True"/"False" to stdout in success.',
        )
    ...
    g.tylibs_path = (
        args.tylibs_path
        or os.environ.get("TYLIBS_PATH")
        or os.path.realpath(os.path.join(os.path.dirname(__file__), ".."))
    )
    os.environ["TYLIBS_PATH"] = g.tylibs_path

    g.tylibs_tools_path = os.environ.get("TYLIBS_TOOLS_PATH") or os.path.expanduser(
        TYLIBS_TOOLS_PATH_DEFAULT
    )
    ...
    if args.tools_json:
        ...
        g.tools_json = os.path.join(g.tylibs_path, TOOLS_FILE)
