...
# tylibs_build_process call.
include(${CMAKE_CURRENT_LIST_DIR}/tylibs.cmake)

# setting PYTHON variable here for compatibility only, new code should use
# tylibs_build_get_property(variable PYTHON)
tylibs_build_get_property(PYTHON PYTHON)
if(NOT PYTHON)
  message(
    FATAL_ERROR "Internal error, PYTHON build property not set correctly.")
endif()

# legacy variable for compatibility
set(TYLIBSTOOL ${PYTHON} "${TYLIBS_PATH}/tools/tylibs.py")
...
# Store CMake arguments that need to be passed into all CMake sub-projects as
# well (bootloader, ULP, etc)
#
# It's not possible to tell if CMake was called with --warn-uninitialized, so to
# also have these warnings in sub-projects we set a cache variable as well and
# then check that.
if(WARN_UNINITIALIZED)
  tylibs_build_set_property(EXTRA_CMAKE_ARGS --warn-uninitialized)
else()
  tylibs_build_set_property(EXTRA_CMAKE_ARGS "")
endif()

# Enable the component manager for regular projects if not explicitly disabled.
if(NOT "$ENV{TYLIBS_COMPONENT_MANAGER}" EQUAL "0")
  tylibs_build_set_property(TYLIBS_COMPONENT_MANAGER 1)
endif()
# Set component manager interface version
tylibs_build_set_property(__TY_COMPONENT_MANAGER_INTERFACE_VERSION 3)

#
# Parse and store the VERSION argument provided to the project() command.
#
function(__parse_and_store_version_arg)
  # The project_name is the first argument that was passed to the project()
  # command
  ...
  # If the VERSION keyword exists but no version string is provided then raise a
  # warning
  if((NOT PROJECT_VERSION OR PROJECT_VERSION STREQUAL "NOTFOUND")
    ...
    # Check if version is valid. cmake allows the version to be in the format
    ...
    # Store the version string in cmake specified variables to access the
    # version
    set(PROJECT_VERSION
        ${PROJECT_VERSION}
        PARENT_SCOPE)
    ...
    # Also store the version string in the specified variables for the
    # project_name
...
#
# Get the project version from a version file. This is passed to the
# tylibs_build_process call. Dependencies are also set here for when the version
# file changes (if it is used).
#
function(__project_get_revision_from_version_file var)
  ...
  if(EXISTS "${_project_path}/version.txt")
    ...
    set_property(
      DIRECTORY
      APPEND
      PROPERTY CMAKE_CONFIGURE_DEPENDS "${_project_path}/version.txt")
  endif()
  set(${var}
      "${PROJECT_VER}"
      PARENT_SCOPE)
...
#
# Replacement for spaces2list in cases where it was previously used on directory
# lists.
#
# If the variable doesn't contain spaces, (e.g. is already a CMake list) then
# the variable is unchanged. Otherwise an external Python script is called to
# try to split the paths, and the variable is updated with the result.
#
...
function(paths_with_spaces_to_list variable_name)
  if("${${variable_name}}" MATCHES "[ \t]")
    ...
    execute_process(
      COMMAND ${python} "${tylibs_path}/tools/split_paths_by_spaces.py"
              "--var-name=${variable_name}" "${${variable_name}}"
      WORKING_DIRECTORY "${CMAKE_CURRENT_LIST_DIR}"
      OUTPUT_VARIABLE result
      RESULT_VARIABLE ret)
    if(NOT ret EQUAL 0)
      message(
        FATAL_ERROR "Failed to parse ${variable_name}, see diagnostics above")
    endif()
    set("${variable_name}"
        "${result}"
        PARENT_SCOPE)
...
function(__ty_build_component_info components output)
  set(components_json "")
  foreach(name ${components})
    __ty_component_get_target(target ${name})
    __ty_component_get_property(alias ${target} COMPONENT_ALIAS)
    __ty_component_get_property(prefix ${target} __PREFIX)
    __ty_component_get_property(dir ${target} COMPONENT_DIR)
    __ty_component_get_property(type ${target} COMPONENT_TYPE)
    __ty_component_get_property(lib ${target} COMPONENT_LIB)
    __ty_component_get_property(reqs ${target} REQUIRES)
    __ty_component_get_property(include_dirs ${target} INCLUDE_DIRS)
    __ty_component_get_property(priv_reqs ${target} PRIV_REQUIRES)
    __ty_component_get_property(managed_reqs ${target} MANAGED_REQUIRES)
    __ty_component_get_property(managed_priv_reqs ${target}
                                MANAGED_PRIV_REQUIRES)
    if("${type}" STREQUAL "LIBRARY")
      set(file "$<TARGET_LINKER_FILE:${lib}>")

      # The tylibs_component_register function is converting each source file
      # path defined in SRCS into absolute one. But source files can be also
      # added with cmake's target_sources and have relative paths. This is used
      # for example in log component. Let's make sure all source files have
      # absolute path.
      set(sources "")
...
function(__all_component_info output)
  ...
  foreach(target ${component_targets})
    __ty_component_get_property(name ${target} COMPONENT_NAME)
    __ty_component_get_property(alias ${target} COMPONENT_ALIAS)
    __ty_component_get_property(prefix ${target} __PREFIX)
    __ty_component_get_property(dir ${target} COMPONENT_DIR)
    __ty_component_get_property(type ${target} COMPONENT_TYPE)
...
#
# Output the built components to the user. Generates files for invoking
# esp_tylibs_monitor that doubles as an overview of some of the more important
# build properties.
#
function(__project_info test_components)
  tylibs_build_get_property(prefix __PREFIX)
  tylibs_build_get_property(_build_components BUILD_COMPONENTS)
  ...
  foreach(build_component ${_build_components})
    __ty_component_get_target(component_target "${build_component}")
    __ty_component_get_property(_name ${component_target} COMPONENT_NAME)
    __ty_component_get_property(_prefix ${component_target} __PREFIX)
    __ty_component_get_property(_alias ${component_target} COMPONENT_ALIAS)
    __ty_component_get_property(_dir ${component_target} COMPONENT_DIR)
  ...
  # __generate_gdbinit() tylibs_build_get_property(gdbinit_files_prefix_map
  # GDBINIT_FILES_PREFIX_MAP) tylibs_build_get_property(gdbinit_files_symbols
  # GDBINIT_FILES_SYMBOLS) tylibs_build_get_property(gdbinit_files_py_extensions
  # GDBINIT_FILES_PY_EXTENSIONS) tylibs_build_get_property(gdbinit_files_connect
  # GDBINIT_FILES_CONNECT) __get_openocd_options(debug_arguments_openocd)
  ...
  make_json_list("${build_component_paths};${test_component_paths}"
                 build_component_paths_json)
  make_json_list("${common_component_reqs}" common_component_reqs_json)

  __ty_build_component_info("${build_components};${test_components}"
                            build_component_info_json)
  __all_component_info(all_component_info_json)

  # The configure_file function doesn't process generator expressions, which are
  # needed e.g. to get component target library(TARGET_LINKER_FILE), so the
  # project_description file is created in two steps. The first step, with
  # configure_file, creates a temporary file with cmake's variables substituted
  # and unprocessed generator expressions. The second step, with file(GENERATE),
  # processes the temporary file and substitute generator expression into the
  # final project_description.json file.
  configure_file("${tylibs_path}/tools/cmake/project_description.json.in"
                 "${build_dir}/project_description.json.templ")
  file(READ "${build_dir}/project_description.json.templ"
       project_description_json_templ)
  file(REMOVE "${build_dir}/project_description.json.templ")
  file(
    GENERATE
    OUTPUT "${build_dir}/project_description.json"
  ...
  # build_component_paths is the paths to all of these components, obtained from
  # the component dependencies file.
  #
...
function(__project_init components_var test_components_var)
  # Use EXTRA_CFLAGS, EXTRA_CXXFLAGS and EXTRA_CPPFLAGS to add more priority
  # options to the compiler EXTRA_CPPFLAGS is used for both C and C++ Unlike
  # environments' CFLAGS/CXXFLAGS/CPPFLAGS which work for both host and target
  # build, these works only for target build
  ...
  tylibs_build_set_property(C_COMPILE_OPTIONS "${extra_cflags}" APPEND)
  tylibs_build_set_property(CXX_COMPILE_OPTIONS "${extra_cxxflags}" APPEND)
  tylibs_build_set_property(COMPILE_OPTIONS "${extra_cppflags}" APPEND)
  ...
  function(__project_component_dir component_dir component_source)
    ...
    # The directory itself is a valid tylibs component
    if(EXISTS ${component_dir}/CMakeLists.txt)
      tylibs_build_component(${component_dir} ${component_source})
    else()
      tylibs_build_get_property(exclude_dirs EXTRA_COMPONENT_EXCLUDE_DIRS)
      # otherwise, check whether the subfolders are potential tylibs components
      file(GLOB component_dirs ${component_dir}/*)
      foreach(component_dir ${component_dirs})
        if(IS_DIRECTORY ${component_dir} AND NOT ${component_dir} IN_LIST
                                             exclude_dirs)
  ...
  # Add component directories to the build, given the component filters,
  # exclusions extra directories, etc. passed from the root CMakeLists.txt.
  if(COMPONENT_DIRS)
    # User wants to fully override where components are pulled from.
    paths_with_spaces_to_list(COMPONENT_DIRS)
    tylibs_build_set_property(__TY_COMPONENT_TARGETS "")
    foreach(component_dir ${COMPONENT_DIRS})
      get_filename_component(component_abs_path ${component_dir} ABSOLUTE)
      if(NOT EXISTS ${component_abs_path})
        message(
          FATAL_ERROR
            "Directory specified in COMPONENT_DIRS doesn't exist: ${component_abs_path}"
        )
      endif()
    ...
    foreach(component_dir ${EXTRA_COMPONENT_DIRS})
      ...
      if(NOT EXISTS ${component_abs_path})
        message(
          FATAL_ERROR
            "Directory specified in EXTRA_COMPONENT_DIRS doesn't exist: ${component_abs_path}"
        )
      endif()
  ...
  # For bootloader components, we only need to set-up the Kconfig files. Indeed,
  # bootloader is currently compiled as a subproject, thus, its components are
  # not part of the main project. However, in order to be able to configure
  # these bootloader components using menuconfig, we need to look for their
  # Kconfig-related files now.
  file(GLOB bootloader_component_dirs
  ...
  foreach(bootloader_component_dir ${bootloader_component_dirs})
    if(IS_DIRECTORY ${bootloader_component_dir})
      __ty_component_dir_quick_check(is_component ${bootloader_component_dir})
      if(is_component)
  ...
  # If a minimal build is requested, set COMPONENTS to "main" only if the
  # COMPONENTS variable is not already defined. The COMPONENTS variable takes
  # precedence over the MINIMAL_BUILD property.
  tylibs_build_get_property(minimal_build MINIMAL_BUILD)
  if(minimal_build)
    if(DEFINED COMPONENTS)
      message(
        WARNING
          "The MINIMAL_BUILD property is disregarded because the COMPONENTS variable is defined."
      )
  ...
  if(TESTS_ALL
     OR BUILD_TESTS
     OR TEST_COMPONENTS
     OR TEST_EXCLUDE_COMPONENTS)
    spaces2list(TEST_COMPONENTS)
    spaces2list(TEST_EXCLUDE_COMPONENTS)
    tylibs_build_get_property(component_targets __TY_COMPONENT_TARGETS)
    foreach(component_target ${component_targets})
      __ty_component_get_property(component_dir ${component_target}
                                  COMPONENT_DIR)
      __ty_component_get_property(component_name ${component_target}
                                  COMPONENT_NAME)
      if(component_name IN_LIST components)
        ...
        if(TEST_EXCLUDE_COMPONENTS AND component_name IN_LIST
                                       TEST_EXCLUDE_COMPONENTS)
          set(include 0)
        endif()
        if(include AND EXISTS ${component_dir}/test)
          __ty_component_add(${component_dir}/test ${component_name}
                             "project_components")
...
macro(project project_name)
  # Initialize project, preparing COMPONENTS argument for tylibs_build_process()
  # call later using external COMPONENT_DIRS, COMPONENTS_DIRS,
  ...
  # If CMAKE_COLOR_DIAGNOSTICS not set in project CMakeLists.txt or in the
  # environment, enable it by default.
  if(NOT DEFINED CMAKE_COLOR_DIAGNOSTICS AND NOT DEFINED
                                             ENV{CMAKE_COLOR_DIAGNOSTICS})
    set(CMAKE_COLOR_DIAGNOSTICS ON)
  endif()

  # Since components can import third-party libraries, the original definition
  # of project() should be restored before the call to add components to the
  # build.
  function(project)
    ...
    #
    # There is some nuance when it comes to setting version variables in terms
    # of whether CMP0048 is set to OLD or NEW. However, the proper behavior
    # should have bee already handled by the original project call, and we're
    # just echoing the values those variables were set to.
    set(PROJECT_NAME
        "${PROJECT_NAME}"
        PARENT_SCOPE)
  ...
  # If PROJECT_VER has not been set yet, look for the version from various
  ...
  #
  # PS: PROJECT_VER will get overridden later if
  # CONFIG_APP_PROJECT_VER_FROM_CONFIG is defined. See
  ...
  if(NOT DEFINED PROJECT_VER)
    ...
    # If the version is not set from the version.txt file, check other sources
    # for the version information
    if(NOT project_ver)
      # Check if version information was passed to project() via the VERSION
      # argument
      ...
      if(version_keyword_present)
        ...
        if(PROJECT_VER_GIT)
          ...
          message(
            STATUS "Could not use 'git describe' to determine PROJECT_VER.")
          # None of sources contain the version information. Default PROJECT_VER
          # to 1.
          set(project_ver 1)
    ...
    # PROJECT_VER has been set before calling project(). Copy it into
    # project_ver for tylibs_build_process() later.
  ...
  # Special treatment for 'main' component for standard projects (not part of
  # core build system). Have it depend on every other component in the build.
  # This is a convenience behavior for the standard project; thus is done
  # outside of the core build system so that it treats components equally.
  #
  # This behavior should only be when user did not set REQUIRES/PRIV_REQUIRES
  # manually.
  tylibs_build_get_property(build_components BUILD_COMPONENT_ALIASES)
  if(tylibs::main IN_LIST build_components)
    __ty_component_get_target(main_target tylibs::main)
    __ty_component_get_property(reqs ${main_target} REQUIRES)
    __ty_component_get_property(priv_reqs ${main_target} PRIV_REQUIRES)
    __ty_component_get_property(managed_reqs ${main_target} MANAGED_REQUIRES)
    __ty_component_get_property(managed_priv_reqs ${main_target}
                                MANAGED_PRIV_REQUIRES)
    # if user has not set any requirements, except ones added with the component
    # manager
    if((NOT reqs OR reqs STREQUAL managed_reqs)
       AND (NOT priv_reqs OR priv_reqs STREQUAL managed_priv_reqs))
      if(test_components)
        list(REMOVE_ITEM build_components ${test_components})
      endif()
      list(REMOVE_ITEM build_components tylibs::main)
      __ty_component_get_property(lib ${main_target} COMPONENT_LIB)
      set_property(
        TARGET ${lib}
        APPEND
        PROPERTY INTERFACE_LINK_LIBRARIES "${build_components}")
      get_property(
        type
        TARGET ${lib}
        PROPERTY TYPE)
      if(type STREQUAL STATIC_LIBRARY)
        set_property(
          TARGET ${lib}
          APPEND
          PROPERTY LINK_LIBRARIES "${build_components}")
      endif()
    endif()
  endif()
  ...
  # Create a dummy file to work around CMake requirement of having a source file
  # while adding an executable. This is also used by esp_tylibs_size to detect
  # the target
  set(project_elf_src ${CMAKE_BINARY_DIR}/project_elf_src_${TYLIBS_TARGET}.c)
  add_custom_command(
    OUTPUT ${project_elf_src}
  ...
  if(CONFIG_TYLIBS_TARGET_LINUX AND CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
    # Compiling for the host, and the host is macOS, so the linker is Darwin LD.
    # Note, when adding support for Clang and LLD based toolchain this check
  ...
  if(test_components)
    ...
    elseif(linker_type STREQUAL "Darwin")
      foreach(test_component ${test_components})
        if(TARGET ${test_component})
          target_link_libraries(${project_elf} PRIVATE "-Wl,-force_load"
                                                       ${test_component})
        endif()
      endforeach()
    endif()
  ...
  foreach(build_component ${build_components})
    __ty_component_get_target(build_component_target ${build_component})
    __ty_component_get_property(whole_archive ${build_component_target}
                                WHOLE_ARCHIVE)
    if(whole_archive)
      if(linker_type STREQUAL "GNU")
        message(
          STATUS
            "Component ${build_component} will be linked with -Wl,--whole-archive"
        )
        target_link_libraries(
          ${project_elf} PRIVATE "-Wl,--whole-archive" ${build_component}
                                 "-Wl,--no-whole-archive")
      elseif(linker_type STREQUAL "Darwin")
        message(
          STATUS
            "Component ${build_component} will be linked with -Wl,-force_load")
        target_link_libraries(${project_elf} PRIVATE "-Wl,-force_load"
                                                     ${build_component})
      endif()
    else()
  ...
  if(linker_type STREQUAL "GNU")
    ...
    set(tylibs_target "${TYLIBS_TARGET}")
    string(TOUPPER ${tylibs_target} tylibs_target)
    # Add cross-reference table to the map file
    target_link_options(${project_elf} PRIVATE "-Wl,--cref")
    # Add this symbol as a hint for esp_tylibs_size to guess the target name
    target_link_options(${project_elf} PRIVATE
                        "-Wl,--defsym=TYLIBS_TARGET_${tylibs_target}=0")
    ...
    execute_process(
      COMMAND ${CMAKE_LINKER} "--no-warn-rwx-segments" "--version"
  ...
  if(CONFIG_LIBC_PICOLIBC)
    tylibs_build_set_property(C_COMPILE_OPTIONS "-specs=picolibc.specs" APPEND)
    tylibs_build_set_property(CXX_COMPILE_OPTIONS "-specs=picolibcpp.specs"
                              APPEND)
    tylibs_build_set_property(LINK_OPTIONS "-specs=picolibc.specs" APPEND)
  endif()

  set_property(
    DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
    APPEND
    PROPERTY ADDITIONAL_CLEAN_FILES "${mapfile}" "${project_elf_src}")

  tylibs_build_get_property(tylibs_path TYLIBS_PATH)
  tylibs_build_get_property(python PYTHON)

  set(tylibs_size ${python} -m esp_tylibs_size)

  # Add size targets, depend on map file, run esp_tylibs_size OUTPUT_JSON is
  # passed for compatibility reasons, SIZE_OUTPUT_FORMAT environment variable is
  # recommended and has higher priority
  add_custom_target(
    size
    COMMAND
      ${CMAKE_COMMAND} -D "TYLIBS_SIZE_TOOL=${tylibs_size}" -D
      "MAP_FILE=${mapfile}" -D "OUTPUT_JSON=${OUTPUT_JSON}" -P
      "${tylibs_path}/tools/cmake/run_size_tool.cmake"
    DEPENDS ${mapfile}
  ...
  add_custom_target(
    ...
    COMMAND
      ${CMAKE_COMMAND} -D "TYLIBS_SIZE_TOOL=${tylibs_size}" -D
      "TYLIBS_SIZE_MODE=--files" -D "MAP_FILE=${mapfile}" -D
      "OUTPUT_JSON=${OUTPUT_JSON}" -P
      "${tylibs_path}/tools/cmake/run_size_tool.cmake"
    DEPENDS ${mapfile}
    USES_TERMINAL VERBATIM)
  ...
  add_custom_target(
    ...
    COMMAND
      ${CMAKE_COMMAND} -D "TYLIBS_SIZE_TOOL=${tylibs_size}" -D
      "TYLIBS_SIZE_MODE=--archives" -D "MAP_FILE=${mapfile}" -D
      "OUTPUT_JSON=${OUTPUT_JSON}" -P
      "${tylibs_path}/tools/cmake/run_size_tool.cmake"
    DEPENDS ${mapfile}
    USES_TERMINAL VERBATIM)
  ...
  set(UF2_CMD ${python} "${tylibs_path}/tools/mkuf2.py" write --chip
              ${chip_model})

  add_custom_target(
    uf2
    COMMAND
      ${CMAKE_COMMAND} -D "TYLIBS_PATH=${tylibs_path}" -D "UF2_CMD=${UF2_CMD}"
      -D "UF2_ARGS=${UF2_ARGS};-o;${CMAKE_CURRENT_BINARY_DIR}/uf2.bin" -P
      "${tylibs_path}/tools/cmake/run_uf2_cmds.cmake"
    USES_TERMINAL VERBATIM)
  ...
  add_custom_target(
    ...
    COMMAND
      ${CMAKE_COMMAND} -D "TYLIBS_PATH=${tylibs_path}" -D "UF2_CMD=${UF2_CMD}"
      -D
      "UF2_ARGS=${UF2_ARGS};-o;${CMAKE_CURRENT_BINARY_DIR}/uf2-app.bin;--bin;app"
      -P "${tylibs_path}/tools/cmake/run_uf2_cmds.cmake"
    USES_TERMINAL VERBATIM)

  tylibs_build_executable(${project_elf})
