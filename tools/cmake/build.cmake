...
#
# @brief Retrieve the value of the specified property related to ESP-TYLIBS
# build.
#
...
#
# @brief Set the value of the specified property related to ESP-TYLIBS build.
# The property is also added to the internal list of build properties if it
# isn't there already.
#
...
function(tylibs_build_set_property property value)
  ...
  tylibs_build_get_property(build_properties __BUILD_PROPERTIES)
  if(NOT property IN_LIST build_properties)
    tylibs_build_set_property(__BUILD_PROPERTIES "${property}" APPEND)
  endif()
endfunction()

# tylibs_build_unset_property
#
# @brief Unset the value of the specified property related to ESP-TYLIBS build.
...
function(tylibs_build_unset_property property)
  ...
  tylibs_build_get_property(
    build_properties __BUILD_PROPERTIES) # remove from tracked properties
  list(REMOVE_ITEM build_properties ${property})
  tylibs_build_set_property(__BUILD_PROPERTIES "${build_properties}")
endfunction()
...
#
function(__build_get_tylibs_git_revision)
...
# common across all library targets built under the ESP-TYLIBS build system.
# These build specifications are added privately using the directory-level CMake
...
#
function(__build_init tylibs_path)

  set(target ${TYLIBS_TARGET})

  # Create the build target, to which the ESP-TYLIBS build properties,
  # dependencies are attached to. Must be global so as to be accessible from any
  # subdirectory in custom projects.
  ...
  __build_get_tylibs_git_revision()
  __kconfig_init()
endfunction()
...
# internal property, __BUILD_COMPONENT_TARGETS.
#
function(__build_expand_requirements component_target)
  ...
  foreach(req ${reqs})
    ...
    __build_resolve_and_add_req(_component_target ${component_target} ${req}
                                __REQUIRES)
  ...
  if(NOT component_target IN_LIST build_component_targets)
    tylibs_build_set_property(__BUILD_COMPONENT_TARGETS ${component_target}
                              APPEND)

    __component_get_property(component_lib ${component_target} COMPONENT_LIB)
    tylibs_build_set_property(__BUILD_COMPONENTS ${component_lib} APPEND)
...
# tylibs_build_process
#
# @brief Main processing step for ESP-TYLIBS build: config generation, adding
# components to the build, dependency resolution, etc.
#
# @param[in] target ESP-TYLIBS target
#
...
macro(tylibs_build_process target)
  ...
  if(NOT "${target}" STREQUAL "linux")
    tylibs_build_set_property(__COMPONENT_REQUIRES_COMMON ${arch} APPEND)
  endif()
  ...
  if(tylibs_component_manager EQUAL 1)
    ...
    foreach(__build_component_target ${build_component_targets})
      __component_get_property(__component_name ${__build_component_target}
                               COMPONENT_NAME)
      __component_get_property(__component_dir ${__build_component_target}
                               COMPONENT_DIR)
  ...
  # All targets built under this scope is with the ESP-TYLIBS build system
  set(ESP_PLATFORM 1)
