...
#
# Priority for new value is: - Existing cmake value (ie set with cmake -D, or
# already set in CMakeLists) - Value of any non-empty environment variable of
# the same name - Default value as provided to function
#
...
# lines2list
#
# Take a variable with multiple lines of output in it, convert it to a cmake
# list (semicolon-delimited), one line per item
#
...
# move_if_different
#
# If 'source' has different md5sum to 'destination' (or destination does not
# exist, move it across.
#
# If 'source' has the same md5sum as 'destination', delete 'source'.
#
# Avoids timestamp updates for re-generated files where content hasn't changed.
...
# target_add_binary_data adds binary data into the built target, by converting
# it to a generated source file which is then compiled to a binary object as
# part of the build
function(target_add_binary_data target embed_file embed_type)
  ...
  add_custom_command(
    ...
    COMMAND
      "${CMAKE_COMMAND}" -D "DATA_FILE=${embed_file}" -D
      "SOURCE_FILE=${embed_srcfile}" ${rename_to_arg} -D
      "FILE_TYPE=${embed_type}" -P
      "${tylibs_path}/tools/cmake/scripts/data_file_embed_asm.cmake"
    MAIN_DEPENDENCY "${embed_file}"
    DEPENDS "${tylibs_path}/tools/cmake/scripts/data_file_embed_asm.cmake"
            ${__DEPENDS}
...
# Append a single line to the file specified The line ending is determined by
# the host OS
...
function(target_linker_script target deptype scriptfiles)
  ...
  foreach(scriptfile ${scriptfiles})
    ...
    # Note: In ESP-TYLIBS, most targets are libraries and libary LINK_DEPENDS
    # don't propagate to executable(s) the library is linked to. Since CMake
    # 3.13, INTERFACE_LINK_DEPENDS is available to solve this. However, when GNU
    # Make generator is used, this property also propagates
    # INTERFACE_LINK_DEPENDS dependencies to other static libraries. TODO: see
    # if this is an expected behavior and possibly report this as a bug to
    # CMake. For the time being, record all linker scripts in __LINK_DEPENDS and
    # attach manually to the executable target once it is known.
    if(NOT __PROCESS)
      tylibs_build_set_property(__LINK_DEPENDS ${abs_script} APPEND)
    endif()
  endforeach()
endfunction()
...
# fail_at_build_time
#
# Creates a phony target which fails the build and touches CMakeCache.txt to
# cause a cmake run next time.
#
# This is used when a missing file is required at CMake runtime, but we can't
# fail the build if it is not found, because the "menuconfig" target may be
# required to fix the problem.
#
# We cannot use CMAKE_CONFIGURE_DEPENDS instead because it only works for files
# which exist at CMake runtime.
#
function(fail_at_build_time target_name message_line0)
  ...
  foreach(message_line ${ARGN})
    set(message_lines ${message_lines} COMMAND ${CMAKE_COMMAND} -E echo
                      "${message_line}")
  endforeach()
  # Generate a timestamp file that gets included. When deleted on build, this
  # forces CMake to rerun.
  ...
  add_custom_target(
    ...
    COMMAND ${CMAKE_COMMAND} -E env FAIL_MESSAGE=${fail_message}
            ${CMAKE_COMMAND} -P ${tylibs_path}/tools/cmake/scripts/fail.cmake
    VERBATIM)
endfunction()

# fail_target
#
# Creates a phony target which fails when invoked. This is used when the
# necessary conditions for a target are not met, such as configuration. Rather
# than ommitting the target altogether, we fail execution with a helpful
# message.
...
#
# This adds global options, set target properties for component-specific flags
...
#
# Add prebuilt library with support for adding dependencies on ESP-TYLIBS
# components.
function(add_prebuilt_library target_name lib_path)
  cmake_parse_arguments(_ "" "" "REQUIRES;PRIV_REQUIRES" ${ARGN})

  get_filename_component(lib_path "${lib_path}" ABSOLUTE BASE_DIR
...
function(file_generate output)
  ...
  set_property(
    DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}"
    APPEND
    PROPERTY ADDITIONAL_CLEAN_FILES "${output}")
endfunction()
...
function(add_subdirectory_if_exists source_dir)
  get_filename_component(abs_dir "${source_dir}" ABSOLUTE BASE_DIR
                         "${CMAKE_CURRENT_LIST_DIR}")
  if(EXISTS "${abs_dir}")
...
function(add_deprecated_target_alias old_target new_target)
  add_custom_target(
    ${old_target}
    # `COMMAND` is important to print the `COMMENT` message at the end of the
    # target action.
    COMMAND ${CMAKE_COMMAND} -E echo ""
    COMMENT
      "Warning: command \"${old_target}\" is deprecated. Have you wanted to run \"${new_target}\" instead?"
  )
