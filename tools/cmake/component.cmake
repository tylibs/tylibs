...
function(__component_dir_quick_check var component_dir)
  ...
  # Check the component directory contains a CMakeLists.txt file - warn and skip
  # anything which isn't valid looking (probably cruft)
  if(NOT first_char STREQUAL ".")
    if(NOT EXISTS "${abs_dir}/CMakeLists.txt")
      message(
        STATUS
          "Component directory ${abs_dir} does not contain a CMakeLists.txt file. "
...
function(__component_add component_dir prefix component_source)
  ...
  # component targets are created early in the build (during adding component as
  # this function suggests). This is so that we still have a target to attach
  # properties to up until the component registration. Plus, interface libraries
  ...
  if(NOT EXISTS "${abs_dir}/CMakeLists.txt")
    message(
      FATAL_ERROR "Directory '${component_dir}' does not contain a component.")
  endif()

  set(component_name ${base_dir})
  # The component target has three underscores as a prefix. The corresponding
  # component library only has two.
  set(component_target ___${prefix}_${component_name})

  # If a component of the same name has not been added before If it has been
  # added before just override the properties. As a side effect, components
  # added later 'override' components added earlier.
  ...
  set(component_alias ${prefix}::${component_name}) # The 'alias' of the
                                                    # component library,
  # used to refer to the component outside the build system. Users can use this
  # name to resolve ambiguity with component names and to link TYLIBS components
  # to external targets.
  ...
  # these two properties are used to keep track of the components known to the
  # build system
  tylibs_build_set_property(BUILD_COMPONENT_DIRS ${component_dir} APPEND)
  tylibs_build_set_property(BUILD_COMPONENT_TARGETS ${component_target} APPEND)
endfunction()

#
# Given a component directory, get the requirements by expanding it early. The
...
function(__component_get_requirements)
  ...
  execute_process(
    COMMAND
      "${CMAKE_COMMAND}" -D "ESP_PLATFORM=1" -D
      "BUILD_PROPERTIES_FILE=${build_properties_file}" -D
      "COMPONENT_PROPERTIES_FILE=${component_properties_file}" -D
      "COMPONENT_REQUIRES_FILE=${component_requires_file}" -P
      "${tylibs_path}/tools/cmake/scripts/component_get_requirements.cmake"
  ...
  if(tylibs_component_manager EQUAL 1)
    ...
    # component_get_requirements.cmake script by adding dependencies defined in
    # component manager manifests to REQUIRES and PRIV_REQUIRES fields. These
    # requirements are also set as MANAGED_REQUIRES and MANAGED_PRIV_REQUIRES
    # component properties.
    execute_process(
      COMMAND
        ${python} "-m" "tylibs_component_manager.prepare_components"
        "--project_dir=${project_dir}" "--lock_path=${DEPENDENCIES_LOCK}"
        "--interface_version=${component_manager_interface_version}"
        "inject_requirements" "--tylibs_path=${tylibs_path}"
        "--build_dir=${build_dir}"
...
macro(__component_add_sources sources)
  ...
  if(__SRCS)
    if(__SRC_DIRS)
      message(
        WARNING "SRCS and SRC_DIRS are both specified; ignoring SRC_DIRS.")
    endif()
    foreach(src ${__SRCS})
    ...
    if(__SRC_DIRS)
      foreach(dir ${__SRC_DIRS})
        get_filename_component(abs_dir ${dir} ABSOLUTE BASE_DIR
                               ${COMPONENT_DIR})
        ...
        file(GLOB dir_sources "${abs_dir}/*.c" "${abs_dir}/*.cpp"
             "${abs_dir}/*.S")
        list(SORT dir_sources)

        if(dir_sources)
          foreach(src ${dir_sources})
            get_filename_component(src "${src}" ABSOLUTE BASE_DIR
                                   ${COMPONENT_DIR})
            list(APPEND sources "${src}")
          endforeach()
        else()
...
macro(__component_check_target)
  if(__REQUIRED_TYLIBS_TARGETS)
    ...
    if(NOT tylibs_target IN_LIST __REQUIRED_TYLIBS_TARGETS)
      message(
        FATAL_ERROR
          "Component ${COMPONENT_NAME} only supports targets: ${__REQUIRED_TYLIBS_TARGETS}"
      )
    endif()
  endif()
endmacro()
    ...
    if(req IN_LIST build_component_targets)
      ...
      if("${type}" STREQUAL "PRIVATE")
        set_property(
          ...
          APPEND
          PROPERTY LINK_LIBRARIES ${req_lib})
        set_property(
          TARGET ${component_lib}
          APPEND
          PROPERTY INTERFACE_LINK_LIBRARIES $<LINK_ONLY:${req_lib}>)
      elseif("${type}" STREQUAL "PUBLIC")
        ...
        set_property(
          TARGET ${component_lib}
          APPEND
          PROPERTY INTERFACE_LINK_LIBRARIES ${req_lib})
      else() # INTERFACE
        set_property(
          TARGET ${component_lib}
          APPEND
...
macro(__component_set_all_dependencies)
  __component_get_property(type ${component_target} COMPONENT_TYPE)
  tylibs_build_get_property(build_component_targets __BUILD_COMPONENT_TARGETS)
...
# tylibs_component_get_property
#
# @brief Retrieve the value of the specified component property
#
# @param[out] var the variable to store the value of the property in @param[in]
# component the component name or alias to get the value of the property of
...
# tylibs_component_set_property
#
# @brief Set the value of the specified component property related. The property
# is also added to the internal list of component properties if it isn't there
# already.
#
# @param[in] component component name or alias of the component to set the
# property of @param[in] property the property to set the value of @param[out]
# value value of the property to set to
#
...
#
# @brief Register a component to the build, creating component library targets
# etc.
#
# @param[in, optional] SRCS (multivalue) list of source files for the component
# @param[in, optional] SRC_DIRS (multivalue) list of source directories to look
# for source files in (.c, .cpp. .S); ignored when SRCS is specified. @param[in,
# optional] EXCLUDE_SRCS (multivalue) used to exclude source files for the
# specified SRC_DIRS @param[in, optional] INCLUDE_DIRS (multivalue) public
# include directories for the created component library @param[in, optional]
# PRIV_INCLUDE_DIRS (multivalue) private include directories for the created
# component library @param[in, optional] LDFRAGMENTS (multivalue) linker script
# fragments for the component @param[in, optional] REQUIRES (multivalue)
# publicly required components in terms of usage requirements @param[in,
# optional] PRIV_REQUIRES (multivalue) privately required components in terms of
# usage requirements or components only needed for functions/values defined in
# its project_include.cmake @param[in, optional] REQUIRED_TYLIBS_TARGETS
# (multivalue) the list of TYLIBS build targets that the component only supports
# @param[in, optional] EMBED_FILES (multivalue) list of binary files to embed
# with the component @param[in, optional] EMBED_TXTFILES (multivalue) list of
# text files to embed with the component @param[in, optional] KCONFIG (single
# value) override the default Kconfig @param[in, optional] KCONFIG_PROJBUILD
# (single value) override the default Kconfig @param[in, optional] WHOLE_ARCHIVE
# (option) link the component as --whole-archive
function(tylibs_component_register)
  ...
  if(NOT __tylibs_component_context)
    message(
      FATAL_ERROR
  ...
  set_property(
    DIRECTORY
    APPEND
    PROPERTY CMAKE_CONFIGURE_DEPENDS "${COMPONENT_DIR}/tylibs_component.yml")
  ...
  tylibs_build_get_property(include_directories INCLUDE_DIRECTORIES
                            GENERATOR_EXPRESSION)
  tylibs_build_get_property(compile_options COMPILE_OPTIONS
                            GENERATOR_EXPRESSION)
  tylibs_build_get_property(compile_definitions COMPILE_DEFINITIONS
                            GENERATOR_EXPRESSION)
  tylibs_build_get_property(c_compile_options C_COMPILE_OPTIONS
                            GENERATOR_EXPRESSION)
  tylibs_build_get_property(cxx_compile_options CXX_COMPILE_OPTIONS
                            GENERATOR_EXPRESSION)
  ...
  if(common_reqs) # check whether common_reqs exists, this may be the case in
                  # minimalistic host unit test builds
  ...
  # processing linker script fragments
  ...
  set(COMPONENT_LIB
      ${component_lib}
      PARENT_SCOPE)
  # COMPONENT_TARGET is deprecated but is made available with same function as
  # COMPONENT_LIB for compatibility.
...
#
# @brief Create mock component with CMock and register it to TYLIBS build
# system.
#
# @param[in, optional] INCLUDE_DIRS (multivalue) list include directories which
# belong to the header files provided in MOCK_HEADER_FILES. If any other include
# directories are necessary, they need to be passed here, too. @param[in,
# optional] MOCK_HEADER_FILES (multivalue) list of header files from which the
# mocks shall be generated. @param[in, optional] REQUIRES (multivalue) any other
# components required by the mock component. @param[in, optional] MOCK_SUBDIR
# (singlevalue) tells cmake where are the CMock generated c files.
#
function(tylibs_component_mock)
  set(options)
  ...
  set(CMOCK_DIR "${TYLIBS_PATH}/components/cmock/CMock")
  ...
  foreach(header_file ${__MOCK_HEADER_FILES})
    ...
    if("${__MOCK_SUBDIR}" STREQUAL "")
      list(APPEND MOCK_GENERATED_HEADERS
           "${MOCK_GEN_DIR}/mocks/Mock${file_without_dir}.h")
      list(APPEND MOCK_GENERATED_SRCS
           "${MOCK_GEN_DIR}/mocks/Mock${file_without_dir}.c")
    else()
      list(APPEND MOCK_GENERATED_HEADERS
           "${MOCK_GEN_DIR}/mocks/${__MOCK_SUBDIR}/Mock${file_without_dir}.h")
      list(APPEND MOCK_GENERATED_SRCS
           "${MOCK_GEN_DIR}/mocks/${__MOCK_SUBDIR}/Mock${file_without_dir}.c")
    endif()
  endforeach()

  file(MAKE_DIRECTORY "${MOCK_GEN_DIR}/mocks")

  tylibs_component_register(SRCS "${MOCK_GENERATED_SRCS}" INCLUDE_DIRS
                            ${__INCLUDE_DIRS} REQUIRES ${__REQUIRES})
  ...
  # This command builds the mocks. First, environment variable UNITY_DIR is set.
  # This is necessary to prevent unity from looking in its own submodule which
  # doesn't work in our CI yet... The rest is a straight forward call to
  # cmock.rb, consult cmock's documentation for more information.
  add_custom_command(
    OUTPUT ${MOCK_GENERATED_SRCS} ${MOCK_GENERATED_HEADERS}
    DEPENDS ruby_found
    COMMAND
      ${CMAKE_COMMAND} -E env "UNITY_DIR=${TYLIBS_PATH}/components/unity/unity"
      ruby ${CMOCK_DIR}/lib/cmock.rb
      -o${CMAKE_CURRENT_SOURCE_DIR}/mock/mock_config.yaml
      ${__MOCK_HEADER_FILES})
endfunction()

# tylibs_component_optional_requires
#
# @brief Add a dependency on a given component only if it is included in the
# build.
#
# Calling tylibs_component_optional_requires(PRIVATE dependency_name) has the
# similar effect to target_link_libraries(${COMPONENT_LIB} PRIVATE
# tylibs::dependency_name), only if 'dependency_name' component is part of the
# build. Otherwise, no dependency gets added. Multiple names may be given.
#
# @param[in]  type of the dependency, one of: PRIVATE, PUBLIC, INTERFACE
# @param[in, multivalue] list of component names which should be added as
# dependencies
#
function(tylibs_component_optional_requires req_type)
  ...
  foreach(req ${optional_reqs})
    if(req IN_LIST build_components)
      tylibs_component_get_property(req_lib ${req} COMPONENT_LIB)
      target_link_libraries(${COMPONENT_LIB} ${req_type} ${req_lib})
    endif()
  endforeach()
endfunction()

# tylibs_component_add_link_dependency
#
# @brief Specify than an ESP-TYLIBS component library depends on another
# component library at link time only.
#
# @note Almost always it's better to use tylibs_component_register() REQUIRES or
...
#
# @param[in, required] FROM Component the dependency is from (this component
# depends on the other component) @param[in, optional] TO Component the
# dependency is to (this component is depended on by FROM). If omitted then the
# current component is assumed. For this default value to work, this function
# must be called after tylibs_component_register() in the component
# CMakeLists.txt file.
function(tylibs_component_add_link_dependency)
  set(single_value FROM TO)
  cmake_parse_arguments(_ "" "${single_value}" "" ${ARGN})

  tylibs_component_get_property(from_lib ${__FROM} COMPONENT_LIB)
  if(__TO)
    tylibs_component_get_property(to_lib ${__TO} COMPONENT_LIB)
  else()
  ...
  set_property(
    TARGET ${from_lib}
    APPEND
    PROPERTY INTERFACE_LINK_LIBRARIES $<LINK_ONLY:${to_lib}>)
endfunction()
...
# require_tylibs_targets
#
# Compatibility function for requiring TYLIBS build targets for 3.xx style
# components.
