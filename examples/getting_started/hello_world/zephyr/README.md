...
static void onHelloWorld(const Event *aEvent) { LOG_INF("Action onHelloWorld() called from TYLiBS"); }

int main(void)
{
    // Subscribe to the TYLiBS using the state table. This registers the module
    // to receive and process events according to the defined rules.
    LOG_WRN("Subscribe module '%s' to TYLiBS", TAG);
    tbSubscribe(TB_SUBSCRIBER(TAG, stateTable, TB_TABLE_ROW_COUNT(stateTable), MAIN_STATE_START));

    while (true)
    {
        LOG_INF("Publish HELLO_WORLD event");
        // Publish the HELLO_WORLD event.  The TYLiBS will then look up this
        ...
        // is being sent with the event.    // publish an event to the TYLiBS
...
[00:00:00.000,000] <wrn> main: Subscribe module 'main' to TYLiBS
[00:00:00.000,000] <inf> main: Publish HELLO_WORLD event
[00:00:00.000,000] <inf> main: Action onHelloWorld() called from TYLiBS
[00:00:01.010,000] <inf> main: Publish HELLO_WORLD event
[00:00:01.010,000] <inf> main: Action onHelloWorld() called from TYLiBS
```
