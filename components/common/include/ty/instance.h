 ...
 */
typedef struct tyInstance tyInstance;
 ...
 *
 * @param[in]     aInstanceBuffer      The buffer for OpenThread to use for allocating the tyInstance structure.
 * @param[in,out] aInstanceBufferSize  On input, the size of aInstanceBuffer. On output, if not enough space for
 *                                     tyInstance, the number of bytes required for tyInstance.
 *
 * @returns  A pointer to the new OpenThread instance.
 *
 * @sa tyInstanceFinalize
 */
tyInstance *tyInstanceInit(void *aInstanceBuffer, size_t *aInstanceBufferSize);
 ...
 * @returns  A pointer to the new OpenThread instance.
 */
tyInstance *tyInstanceInitMultiple(uint8_t aIdx);
 ...
 */
uint8_t tyInstanceGetIndex(tyInstance *aInstance);
 ...
 */
uint32_t tyInstanceGetId(tyInstance *aInstance);

/**
 * Indicates whether or not the instance is valid/initialized.
 *
 * The instance is considered valid if it is acquired and initialized using either `tyInstanceInitSingle()` (in single
 * instance case) or `tyInstanceInit()` (in multi instance case). A subsequent call to `tyInstanceFinalize()` causes
 ...
 */
bool tyInstanceIsInitialized(tyInstance *aInstance);
 ...
 */
uint64_t tyInstanceGetUptime(tyInstance *aInstance);
 ...
 */
void tyInstanceGetUptimeAsString(tyInstance *aInstance, char *aBuffer, uint16_t aSize);
 ...
 * @param[in]  aContext    A pointer to application-specific context.
 */
void otRemoveStateChangeCallback(tyInstance *aInstance, otStateChangedCallback aCallback, void *aContext);
 ...
 * `tylibsReset` does not erase any persistent state/info saved in non-volatile memory.
 *
 * @param[in]  aInstance  A pointer to an OpenThread instance.
 */
 ...
 * @param[in]  aInstance  A pointer to an OpenThread instance.
 */
void tyInstanceFactoryReset(tyInstance *aInstance);
 ...
 * @param[in]  aInstance  A pointer to an OpenThread instance.
 */
void tyInstanceResetRadioStack(tyInstance *aInstance);
 ...
 */
const char *otGetRadioVersionString(tyInstance *aInstance);
