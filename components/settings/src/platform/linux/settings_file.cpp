...
tyError SettingsFile::Init(const char *aSettingsFileBaseName)
{
    tyError     error     = TY_ERROR_NONE;
    const char *directory = TY_CONFIG_LINUX_SETTINGS_PATH;
...
tyError SettingsFile::Get(uint16_t aKey, int aIndex, uint8_t *aValue, uint16_t *aValueLength)
{
...
tyError SettingsFile::Delete(uint16_t aKey, int aIndex)
{
    return Delete(aKey, aIndex, nullptr);
}

tyError SettingsFile::Delete(uint16_t aKey, int aIndex, int *aSwapFd)
{
    tyError error = TY_ERROR_NOT_FOUND;
    off_t   size;
    off_t   offset;
...
{
    snprintf(aFileName, kMaxFilePathSize, TY_CONFIG_LINUX_SETTINGS_PATH "/%s.%s", mSettingFileBaseName,
             (aSwap ? "Swap" : "data"));
}
