...
public:
    ...
    int getRemoteFileDescriptor() const { return fileDescriptors_[writeDirection_]; }

    int getLocalFileDescriptor() const { return fileDescriptors_[!writeDirection_]; }
...
protected:
    static void createSocketPair(bool writeDirection, int (&fds)[2])
    {
        ...
        {
            throw sdbus::Error(sdbus::Error::Name("org.bluez.Error.Failed"), strerror(errno));
        }
    }
